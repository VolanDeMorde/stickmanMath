...
// On-screen control buttons need to work with mouse, touch, and pen input.
function bindControlHold(buttonId, key) {
...
function bindControlTap(buttonId, callback) {
    const el = document.getElementById(buttonId);
    if (!el) return;
...
bindControlHold('leftTouch', 'a');
bindControlHold('rightTouch', 'd');
bindControlHold('jumpTouch', ' ');
bindControlTap('grabTouch', () => toggleGrab());
bindControlTap('swingTouch', () => {
    if (playerHeldWeapon) triggerStrike();
});
bindControlTap('breakTouch', () => breakSelectedBlock());
